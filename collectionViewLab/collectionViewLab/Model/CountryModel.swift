...
struct CountryElement: Codable {
    ...
    let cioc: String?
    static func decodeCountryFromData(from jsonData: Data) throws -> [CountryElement] {
        let response = try JSONDecoder().decode([CountryElement].self, from: jsonData)
        return response
    }
}
...
typealias CountryArray = [CountryElement]
